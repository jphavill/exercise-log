...
if find "$CACHE_BASE_DIR" -ignore_readdir_race -maxdepth 0 >/dev/null 2>&1; then
  FIND_RACE_ARGS=(-ignore_readdir_race)
fi

npm_cache_dir="$CACHE_BASE_DIR/npm"
if [ -d "$npm_cache_dir" ]; then
  find "$npm_cache_dir" "${FIND_RACE_ARGS[@]}" -depth -mindepth 1 -mtime +"$CACHE_TTL_DAYS" -exec rm -rf -- {} +
fi

frontend_modules_cache_dir="$CACHE_BASE_DIR/frontend-node_modules"
if [ -d "$frontend_modules_cache_dir" ]; then
  find "$frontend_modules_cache_dir" "${FIND_RACE_ARGS[@]}" -depth -mindepth 1 -maxdepth 1 -type d -mtime +"$CACHE_TTL_DAYS" -exec rm -rf -- {} +
fi
